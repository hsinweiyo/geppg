...
def find_closest(target, target_mid, real_traj):
    ...
    if task == 'goal':
        ...
        pos[0], pos[1] = reacher_target_position(target)
        ...
        pos = np.zeros(2)
        real_traj = np.array(real_traj)[:,:2]
        mid_pos[0], mid_pos[1] = reacher_target_position(target_mid)
        pos[0], pos[1] = reacher_target_position(target)
...
def plot_fig(traj_dict):
    for key in traj_dict:
        ...
        names = key.split('_')
        target_x, target_y = reacher_target_position(names[1])
        plt.plot(traj_dict[key][:,0], traj_dict[key][:,1], target_x, target_y, 'ro')
        if task == 'traj':
            mid_x, mid_y = reacher_target_position(names[2])
...
if __name__ == '__main__':
    ...
    for i in range(nb_eps):
        ...
        if task == 'goal':
            ...
            ideal_pos = [reacher_target_position(target)]
        else:
            goal = eval_dist_cem(target_mid-5, target)
            ideal_pos = np.concatenate((reacher_target_position(target_mid), reacher_target_position(target))) 
        #goal = ideal_pos
